...
void options_manager::add_options_interface()
{
    ...
    add("显示玩家剩余的行动点", "interface", to_translation("显示玩家剩余的行动点"), to_translation("当此选项的值为 是 时，会在游戏画面上显示玩家的剩余的行动点。关闭此选项可以提升游戏性能。"), false);
...
static void update_options_cache()
{
    ...
    use_show_player_move_point = ::get_option<bool>("显示玩家剩余的行动点");
}
