...
bool avatar_action::move( avatar &you, map &m, const tripoint &d )
{
    ...
    if( monster *const mon_ptr = creatures.creature_at<monster>( dest_loc, true ) ) {
        ...
        if( critter.friendly == 0 &&
            ...
            if(critter.is_dead_state()&critter.Lv>3) {  // 如果玩家击杀了精英怪物，那么可以获得1/3的怪物积累的经验

                you.kill_xp=you.kill_xp+int(critter.exp/3);

            }

             // 标记 如果玩家骑着宠物，那么宠物是可以攻击玩家所攻击的对象的      
            if(you.is_mounted()) {
                monster *mon_mounted =you.mounted_creature.get();  // 标记 获取 玩家正在骑乘的怪物
                if(!mon_mounted->has_flag(MF_MILITARY_MECH)) {     // 如果玩家没有骑乘机甲
                        if(mon_mounted->melee_attack(critter))  { 
                            
                            mon_mounted->exp++;   // 标记 玩家与宠物骑乘状态下一起攻击敌人，宠物也可以再每一次成功的攻击后获得经验   
                            
                            int beginLv=mon_mounted->Lv;  // 标记 获取怪物未经处理的Lv
                           
                            if(mon_mounted->Lv!=10) {   // 标记 升级判定与处理
                                
                               if(mon_mounted->exp>= 100 && mon_mounted->Lv==0) {

                                   mon_mounted->Lv++;

                                }  
                                if(mon_mounted->exp>= 500 && mon_mounted->Lv==1) {      
                                
                                    mon_mounted->Lv++;
                                }  
                                if(mon_mounted->exp>= 1500 && mon_mounted->Lv==2) {
                                
                                    mon_mounted->Lv++;
                                }  
                                if(mon_mounted->exp>= 3000 && mon_mounted->Lv==3) {
                                
                                    mon_mounted->Lv++;
                                }   
                                if(mon_mounted->exp>= 5000 && mon_mounted->Lv==4) {
                                
                                    mon_mounted->Lv++;
                                }  
                                if(mon_mounted->exp>= 7000 && mon_mounted->Lv==5) {
                                
                                   mon_mounted->Lv++;
                                }  
                                if(mon_mounted->exp>= 9000 && mon_mounted->Lv==6) {
                                
                                    mon_mounted->Lv++;

                                }  
                                if(mon_mounted->exp>= 11000 && mon_mounted->Lv==7) {
                                
                                    mon_mounted->Lv++;

                                }  
                                if(mon_mounted->exp>= 13000 && mon_mounted->Lv==8) {
                                
                                    mon_mounted->Lv++;

                                }  
                                if(mon_mounted->exp>= 15000 && mon_mounted->Lv==9) {
