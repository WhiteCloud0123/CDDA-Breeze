...
cata::optional<int> iuse::solarpack(Character* p, item* it, bool t, const tripoint&)
{
...
cata::optional<int> iuse::solarpack_off(Character* p, item* it, bool t, const tripoint&)
{
    if (t) {
        return cata::nullopt;
    }
    ...
    *p->as_avatar(), _( "Scan which book?" ), PICKUP_RANGE );
    ...
    for (npc &n :g->all_npcs()) {
        if (i->is_owned_by(n) && n.sees(get_player_character())) {
            message = n.get_name() + "非常严肃地要求你不要做多余的举动。确定要进行扫描吗？";
            if (query_yn(message)) {
