...
void monster::load( const JsonObject &data )
{
    ...
    std::string sidtmp;
    // load->str->int
    data.read("Lv",Lv);      // 标记 读取 Lv
    data.read("exp",exp);    // 标记 读取 exp
    data.read("trySetLv",trySetLv);
...
void monster::store( JsonOut &json ) const
{
    ...
    json.member("Lv",Lv);    // 标记 添加 json成员 Lv
    json.member("exp",exp);  // 标记 添加 json成员 exp
    json.member("trySetLv",trySetLv); // // 标记 添加 json成员 trySetLv
