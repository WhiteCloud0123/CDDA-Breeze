...
static const species_id species_ROBOT( "ROBOT" ); //  标记  species_id species_ROBOT( "ROBOT" )
...
bool monexamine::pet_menu( monster &z )
{
    enum choices {
        ...
        神经渗透,     // 标记 添加枚举项，神经渗透
        Hp,          // 标记 添加枚举项，HP，
        Dodge,       // 标记 添加枚举项，Dodge
        MeleeSkill,  // 标记 添加枚举项 ，MeleeSkill
        Speed,       // 标记 添加枚举项，Speed
        Morale,      // 标记 添加枚举项，Morale
        命令攻击      // 标记 添加枚举项，命令攻击 
    };
    ...
    if(get_player_character().has_trait(trait_THRESH_SLIME)) {   // 标记  如果玩家是液态人，在怪物检查菜单中添加神经渗透、查看属性
    
 
        if(z.in_species(species_ROBOT)==false) {   // 标记 仅仅非机器人才可以应用"神经渗透"和"查看属性"操作
         
           amenu.addentry( 神经渗透, true, 'N', _( "神经渗透" ) );
           amenu.addentry( 查看属性, true, 'C', _( "查看属性" ) );
    ...
    uilist mon_attribute_menu;   // 标记 声明怪物属性菜单
    ...
    switch( choice ) {
        ...
        case talk_to:
            ...
            break;
        case 神经渗透:  {   // 标记 如果选择了 神经渗透  进行相应处理
           
            if(query_yn( _( "真的要进行神经渗透吗?" ))) {
                   
                   Character &player_character=get_player_character();
                
                   int intelligent=player_character.get_int();  // 标记 获取玩家的智力
                   int randInt=NULL; 
                   int speedCount=NULL;   
                   int monDodgeBonus=NULL;
                   int mon_speed_bonus=NULL;
                   int monDodge=NULL;
                   int monSpeed=NULL;
                   ...
                   speedCount=2*intelligent;    // 标记 速度改造点数为2倍的玩家智力
             
                   mon_speed_bonus=0;      // 标记 速度加成为0
                   failInt=0;
                   baseSpeed=100+speedCount;
                
              
                
                   
                   
                   srand(time(NULL));  // 标记 利用当前时间作为随机数种子
                   
                   randInt=rand()%speedCount;
                   
                   chance=rand()%2;         
                   
                   if(chance==0) {
                   
                   mon_speed_bonus=-randInt;
                   
                        if(baseSpeed+mon_speed_bonus<=0) {

                            mon_speed_bonus=0;
                            

                        }
                        add_msg( m_info, _( "这只是一瞬间,却仿佛循环往复绕了山脉一圈又一圈......" ));
                        ...
                        if(failInt==0) {
                            ...
                            z.set_hp(-2147483647);
                            add_msg( m_bad, _( "你失误了!不明的巨大压力在 %s 的体内迅速向外释放!" ),pet_name);
                    ...
                    mon_speed_bonus=randInt;
                    add_msg( m_info, _( "这只是一瞬间,却仿佛过了很长的时间......" ));

                   
                   }
                  
                 
                  z.set_speed_base(baseSpeed+mon_speed_bonus);

                  player_character.moves=player_character.moves-30;  // 标记 神经渗透消耗的行动点数为30
                
         }
             break;
        }
        case 查看属性: {   // 标记  如果选择查看属性，那么进行相应处理 
             
           
            int mon_hp=NULL;           
            int mon_speed=NULL;
            int mon_melee_skill=NULL;
            int monDodge=NULL;
            int monAggressive=NULL;
            int monMorale=NULL;
            int monArmorBash=NULL;
    
            int mon_hp_json=NULL;
            int mon_speed_json=NULL;
            int mon_melee_skill_json=NULL;
            int monDodgeJson=NULL;
            int monAggressiveJson=NULL;
            int monMoraleJson=NULL;
            int monArmorBashJson=NULL;   

            mon_hp=z.get_hp();          
            mon_speed=z.get_speed();
            mon_melee_skill=z.get_melee();
            monDodge=z.get_dodge();


            mon_hp_json=z.get_hp_max();
            mon_speed_json=z.get_speed_base();
            monDodgeJson=z.get_dodge_base();
            mon_melee_skill_json=z.get_hit_base();
            monMoraleJson=z.morale;
            
        
            
            

            mon_attribute_menu.text=string_format( _( "%s 的属性" ), pet_name );           
            mon_attribute_menu.addentry( Hp, true, '1', _( "HP  %1s/%2s" ),mon_hp,mon_hp_json );
            mon_attribute_menu.addentry( Speed, true, '2', _( "Speed  %s" ),mon_speed);
            mon_attribute_menu.addentry( MeleeSkill, true, '3', _( "Melee Skill  %s" ),mon_melee_skill_json );
            mon_attribute_menu.addentry( Dodge, true, '4', _( "Dodge  %1s/%2s" ),monDodge,monDodgeJson );
            mon_attribute_menu.addentry( Morale, true, '5', _( "Morale  %s" ),monMoraleJson );
       //     mon_attribute_menu.addentry( MeleeSkill, true, '3', _( "Melee Skill  %1s/%2s" ),mon_melee_skill,mon_melee_skill_json );
            
            mon_attribute_menu.query();  
