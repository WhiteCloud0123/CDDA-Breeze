...
bool mattack::ranged_pull( monster *z )
{
    ...
    bool seen = get_player_view().sees( *z );
     ...
     // 标记 添加判断，如果玩家在驾驶机甲，无法成为怪物的ranged_pull有效目标
    if(target->is_monster()) {

        monster *mon_target=dynamic_cast< monster * >( target );

        monster *mon_mounted = get_player_character().mounted_creature.get();

        if(mon_target==mon_mounted) {

                 if(mon_mounted->has_flag(MF_MILITARY_MECH)) {

                 add_msg( m_good, _( "%s 无法将机甲中的你拖拽出来"),z->name());
