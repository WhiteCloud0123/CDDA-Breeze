...
static const species_id species_SLIME( "SLIME" );   // 标记 添加 static const species_id species_SLIME( "SLIME" )
static const species_id species_HUMAN( "HUMAN" );   // 标记 添加 species_id species_HUMAN( "HUMAN" );
static const species_id species_ZOMBIE( "ZOMBIE" ); // 标记 添加 species_id species_ZOMBIE( "ZOMBIE" );
static const trait_id trait_THRESH_SLIME( "THRESH_SLIME" );  // 标记 添加 trait_id trait_THRESH_SLIME
static const trait_id trait_Slime_Pretend( "Slime_Pretend" ); // 标记 添加 trait_id trait_Slime_Pretend( "Slime_Pretend" )
static const trait_id trait_Zombie_Pretend( "Zombie_Pretend" ); //标记 添加 trait_id trait_Zombie_Pretend( "Zombie_Pretend" )
static const trait_id trait_Feral_Human_Pretend( "Feral_Human_Pretend" ); // 标记 添加 trait_id trait_CRASYHuman_Pretend( "Feral_Human_Pretend" );
...
void game::mon_info_update( )
{
    ...
    for( Creature *c : u.get_visible_creatures( MAPSIZE_X ) ) {
        ...
        if( m != nullptr ) {
            ...
            // 标记 尝试将玩家视野中的怪物设置成精英怪物，100分之1的概率
            if(critter.trySetLv==false) {

                // 标记 如果玩家有认知性变形怪伪装这个开局变异，变形怪会在一定时间内不会主动攻击玩家
                if( critter.in_species( species_SLIME ) && ( u.has_trait(trait_Slime_Pretend)  ) ) {
                                 
                                 critter.anger=-99;
            
                 }   
                 if( critter.in_species( species_ZOMBIE ) && ( u.has_trait(trait_Zombie_Pretend)  ) ) {
                                 
                                 critter.anger=-99;
            
                 }   
                 if( critter.in_species( species_HUMAN ) && ( u.has_trait(trait_Feral_Human_Pretend)  ) ) {
                                 
                                 critter.anger=-99;
            
                 }   
                
                critter.trySetLv=true;
                ...
                if(randInt==1) {
                    ...
                    int setLv=rng(4,10);  // 在4-10级的范围中，设置精英怪物的等级

                    critter.Lv=setLv;     // 设置等级
            ...
            if(critter.Lv>3&critter.has_effect(effect_pet)==false) {  // 标记 如果怪物的等级大于3并且不是宠物，那么不断的输出警告信息
...
bool game::npc_menu( npc &who )
{
    ...
    amenu.addentry( 表达爱意, true, 'E', _( "表达爱意" ) ); // 标记 添加表达爱意选项

    if(who.op_of_u.trust>9000) {  // 标记 如果 npc对玩家的信任大于9000，那么显示此不可用的选项

         amenu.addentry( 这是你的恋人, false, 'L', _( "这是你的恋人♥" ) );
    ...
    }else if(choice==表达爱意) {     // 标记 添加此判断处理 如果选择为表达爱意，进行响相应处理   
        
        srand(time(NULL));  // 利用当前时间作为随机数种子
    ...
    //    uilist love_menu;
    //    love_menu.title = _( "这是测试菜单" );
    //    love_menu.text=_("输出测试信息");

    //    love_menu.w_width=200;
    //    love_menu.w_height=200;
    //    love_menu.w_x=0;
    //    love_menu.w_y=0;
    //    //love_menu.addentry( 这是你的恋人, false, 'l', _( "这是你的恋人♥" ) );
    //    love_menu.query(true);
    ...
    // Set up main window
        /*SDL_Window* mainWindow = SDL_CreateWindow( "测试", mainPosX, mainPosY, mainSizeX, mainSizeY, 0 );
        ...
        // SDL_Surface *npc_image = NULL;
        // npc_image = IMG_Load("./Test.png");
        
        


        if(query_yn( _( "真的要向对方表达爱意吗?" ))) {
                    
            Character &player_character=get_player_character();
            
            if(who.op_of_u.fear>20) {    // 如果npc过于恐惧，表达心意无法进行

                beginInt=0;

                randInt=rand()%2;

                finalInt=beginInt+randInt;

                
                if(finalInt==0) {

                   add_msg( m_bad, _( "%s 看起来很害怕你......" ),who.get_name() );

                }else {
                   add_msg( m_bad, _( "%s 的眼睛不敢直视你......" ),who.get_name() );

                }

                   
                   
                   add_msg( m_bad, _( "%s 好像更害怕你了......" ),who.get_name() );
                   
                   who.op_of_u.fear=who.op_of_u.fear+5;
            ...
            } else if(who.op_of_u.anger>20) {  // 如果npc过于生气，表达心意无法进行

                add_msg( m_bad, _( "%s 现在非常生气，不想听你说的任何话" ),who.get_name() );
                add_msg( m_bad, _( "%s 好像更生气......" ),who.get_name() );
                
                who.op_of_u.anger++;

            } 
            
            else if(who.op_of_u.trust>50&&who.op_of_u.value>50) {
                    ...
                    if(finalInt==1) {   // 有十分之一几率触发特殊的对话，
                       add_msg( m_good, _( "%1s 也很喜欢你,%2s 随后向你回了一吻......" ),who.get_name(),who.get_name() );
                    }else if(finalInt==2) {                      
                       add_msg( m_good, _( "%s 非常的倾慕你,不断的表达自己对你的爱与赞许......" ),who.get_name() );
                    }else if(finalInt==3) {
                       add_msg( m_good, _( "%s 朝你微笑,然后牵起了你的手......" ),who.get_name() );
                    }else if(finalInt==4) {
                        add_msg( m_good, _( "%s 含情脉脉的看着你,伴随着轻柔又略显急促的吐息,慢慢的贴近你......" ),who.get_name() );
                        if(! query_yn( _( "你们相拥在一起,感受到的是在这末世里少有的珍贵的温暖。那么确定要就此分开吗？毕竟末世的威胁不会一刻停歇......" ))) {
                           
                           who.op_of_u.trust=9999;
                           who.op_of_u.value=9999;
                           who.op_of_u.fear=-1000;
                           who.op_of_u.anger=-1000;
                           who.personality.bravery=who.personality.bravery+50;
                           
                       
                           
                           player_character.fall_asleep(1_hours);

                           add_msg( m_good, _( "你想起了“失去”,也想起了更多的事情......" ),who.get_name() );
                           add_msg( m_good, _( "期待可以慢慢地走向一个好些的结局" ),who.get_name() );

                        }
                    
                    }else if(finalInt==5) {
                       add_msg( m_good, _( "%s 给了你短暂的一吻......" ),who.get_name() );
                    }else if(finalInt==6) {
                       add_msg( m_good, _( "%s 和你一起展望了未来,你们甚至描述起了你们和孩子一起生活的模样......" ),who.get_name() );
                    }else if(finalInt==7) {
                       add_msg( m_good, _( "你轻轻揉了揉了 %1s 的头, %2s 发出了嘻嘻的笑声" ),who.get_name(), who.get_name());
                    }else if(finalInt==8) {
                       add_msg( m_good, _( "你轻轻牵起 %s 的手" ),who.get_name() );
                    }
                    else {
                       
                       add_msg( m_good, _( "%s 稍微抚摸了一下你的脸......" ),who.get_name() );

                    }
                    
                    if(who.op_of_u.trust>9000) {

                        player_character.add_morale( MORALE_LOVE, 1000, 2000, 2_hours, 1_minutes, false);  // 增加心情

                    }
                    
             
             
            }else if(who.op_of_u.trust<=10&&who.op_of_u.value<=10) {
                    ...
                    if(finalInt==0) {
                        ...
                        add_msg( m_bad, _( "%s 冷漠地瞪了你一眼......" ),who.get_name() );

                    }

                    if(finalInt==1) {
                       
                       add_msg( m_bad, _( "%s 隐藏着自己的惊讶,并表示你不要再开玩笑了" ),who.get_name() );

                    }

                    if(finalInt==2) {

                       add_msg( m_bad, _( "%s 说“我觉得这不是我们现在应该谈论的事情......”" ),who.get_name() );
                    
                    }
                    
                    if(finalInt==3) {

                       add_msg( m_bad, _( "%s 说：“我正在观察周围环境是否有威胁，请不要打扰我。”" ),who.get_name() );
                    ...
                    beginInt=0;     // 再次进行一轮生成随机数

                    randInt=rand()%4;

                    finalInt=beginInt+randInt;

                    if(finalInt==0) {  // 糟糕的表达爱意会有四分之一概率降低你在对方心中的评价
                       
                        add_msg( m_bad, _( "你在 %s 心中的评价降低了" ),who.get_name() );
        
                        who.op_of_u.trust--;   // 降低npc的信任
                
                        who.op_of_u.anger++;   // 增加npc对你的愤怒

                        who.op_of_u.fear++;    // 增加npc对你的恐惧

                      
               
                       if(who.op_of_u.value>0) {  // 降低你在npc心中的价值
                    ...
                    } else if(finalInt==1) {  // 四分之一概率严重降低npc对玩家的评价
                        add_msg( m_bad, _( "你在 %s 心中的评价降低了" ),who.get_name() );
        
                        who.op_of_u.trust=who.op_of_u.trust-5;   // 降低npc的信任
                
                        who.op_of_u.anger=who.op_of_u.anger+5;   // 增加npc对你的愤怒


                        if(who.op_of_u.value>0) {  // 降低你在npc心中的价值
               
                        who.op_of_u.value=who.op_of_u.value-5;    

                    }
                    
                    }else if(player_character.get_str()>15||player_character.get_int()>15)  {  //  如果玩家的智力或者力量十分出众，会有四分之三的机会几率使得NPC对自己的value提升

                          
                          who.op_of_u.value= who.op_of_u.value+2;  

                          add_msg( m_good, _( "你在 %s 心中的评价提升了" ),who.get_name() );
            ...
            } else if(who.op_of_u.trust>10&&who.op_of_u.trust<=20) {
                ...
                if(finalInt==0) {
                    ...
                    add_msg( m_info, _( "%s 以为你在开玩笑,朝你挥了挥手,随后转过身子去" ),who.get_name() );
                      
                } else if(finalInt==1) {
                   
                    add_msg( m_info, _( "%s 急忙岔开了话题" ),who.get_name() );

                }else if(finalInt==2) {
                   
                    add_msg( m_info, _( "%s 装作没有听见" ),who.get_name() );

                }
                 
               

                if(who.op_of_u.value>20) {
                     
                     add_msg( m_info, _( "你感觉到 %s 只是觉得你自身有些价值罢了" ),who.get_name() );


                }

            } else if(who.op_of_u.trust>20&&who.op_of_u.trust<=30) {
                 
                add_msg( m_good, _( "%s 脸微微泛红,想说什么，最后还是什么都没有说......" ),who.get_name() );
            ...
            } else if(who.op_of_u.trust>30&&who.op_of_u.trust<=40) {

                add_msg( m_good, _( "%s 称赞起你来......" ),who.get_name() );
                add_msg( m_good, _( "你感觉好像又被搪塞过去了......" ),who.get_name() );
                
                if(who.op_of_u.value<10) {
                     
                     add_msg( m_good, _( "%s 微笑起来,之后谈论起你的不足,并鼓励了你" ),who.get_name() );
            ...
            }else if(who.op_of_u.trust>40&&who.op_of_u.trust<=50) {
                ...
                add_msg( m_good, _( "%s 的脸更红了......" ),who.get_name() );
                add_msg( m_good, _( "%s 为了掩饰自己的害羞，轻轻推了一下你......" ),who.get_name() );

                }else {

                   add_msg( m_good, _( "%s 脸一红,随即岔开话题，搪塞了过去" ),who.get_name() );
            ...
            if(who.op_of_u.trust>50&&who.op_of_u.value<50) {   // 如果满足此判定，都会有百分之1的概率，提升在对方心目中的价值
            ...
            if(who.op_of_u.fear>30) {   // 当对玩家的恐惧感大于30时，表达爱意执行完毕后，npc会逃跑。
                ...
                if( finalInt==0) {
                      ...
                      add_msg( m_bad, _( "%s 过于害怕你，于是逃跑了" ),who.get_name() );
                      who.set_attitude( NPCATT_FLEE);
...
void game::fling_creature( Creature *c, const units::angle &dir, float flvel, bool controlled )
{
    ...
    while( range > 0 ) {
        ...
        if( animate && ( seen || u.sees( *c ) ) ) {
            ...
            // 标记 不会使得smash攻击不会使得玩家与机甲脱离    
        if(c->is_monster()&&!c->is_dead_state()&&get_player_character().is_mounted()) {   
            
            monster *monTarget=dynamic_cast< monster * >( c );
            
            monster *monMounted = get_player_character().mounted_creature.get();

            if(monTarget==monMounted&&monTarget->has_flag(MF_MILITARY_MECH)) {

                    get_player_character().setpos(c->pos());
