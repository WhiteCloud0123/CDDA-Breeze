...
static constexpr int RIVER_Z = 0;

static std::vector<point_abs_om> highway_orthogonal_line_to( const point_abs_om &p1,
        const point_abs_om &p2 )
{
    std::vector<point_abs_om> points;
    points.reserve( manhattan_dist( p1, p2 ) + 1 );
    point_abs_om iter = p1;
    points.emplace_back( iter );
    while( iter.x() != p2.x() ) {
        iter.x() += iter.x() < p2.x() ? 1 : -1;
        points.emplace_back( iter );
    }
    while( iter.y() != p2.y() ) {
        iter.y() += iter.y() < p2.y() ? 1 : -1;
        points.emplace_back( iter );
    }
    return points;
}
                                    ...
                                    om_direction::type segment_dir )
{
    ...
    { tripoint_zero, tripoint_zero, tripoint_east, tripoint_south };
                                    ...
                                    om_direction::type special_dir )
{
    ...
    { tripoint_zero, tripoint_east, tripoint_south_east, tripoint_south };
        ...
        int dir1, int dir2, int base_z )
{
    ...
    add_msg_debug( debugmode::DF_OVERMAP, "overmap (%s) attempting to draw bend from %s to %s.",
...
std::pair<bool, std::bitset<HIGHWAY_MAX_CONNECTIONS>> overmap::highway_handle_oceans()
{
    ...
    if( get_option<bool>( "OVERMAP_PLACE_OCEANS" ) ) {
        const overmap_ocean_settings &settings_ocean = settings->overmap_ocean;
        // In this branch zero disables the corresponding ocean edge.
        const int ocean_start_north = settings_ocean.ocean_start_north > 0 ? settings_ocean.ocean_start_north : INT_MAX;
        const int ocean_start_east = settings_ocean.ocean_start_east > 0 ? settings_ocean.ocean_start_east : INT_MAX;
        const int ocean_start_west = settings_ocean.ocean_start_west > 0 ? settings_ocean.ocean_start_west : INT_MAX;
        const int ocean_start_south = settings_ocean.ocean_start_south > 0 ? settings_ocean.ocean_start_south : INT_MAX;
    ...
    const std::vector<const overmap *> &neighbor_overmaps )
{
    highway_intersection_grid &highway_grid =
        overmap_buffer.highway_intersections;
    ...
    if( !is_highway_neighbors ) {
        add_msg_debug( debugmode::DF_OVERMAP, "This overmap (%s) is NOT a highway overmap.",
                       loc.to_string_writable() );
        return paths;
    } else {
...
void overmap::finalize_highways( std::vector<Highway_path> &paths )
{
    ...
    for( Highway_path &path : paths ) {
        ...
        for( const intrahighway_node &node : path ) {
            ...
            for( overmap_special_locations &loc : locs ) {
                if( ter( node.path_node.pos + om_direction::rotate( loc.p, effective_dir ) )->is_highway_special() ) {
                    is_special = true;
                    break;
                }
            }
...
std::optional<std::bitset<HIGHWAY_MAX_CONNECTIONS>> overmap::is_highway_overmap() const
{
    ...
    const point_abs_om & adjacent ) {
        ...
        std::vector<point_abs_om> highway_oms = highway_orthogonal_line_to( front_back.first, front_back.second );

        std::string path_debug_output;
        for( const point_abs_om &p : highway_oms ) {
            path_debug_output += p.to_string_writable() + " ";
        }
        add_msg_debug( debugmode::DF_OVERMAP, "for %s to %s, checking path: %s",
...
void highway_intersection_grid::generate_offset( overmap_feature_grid_node &node )
{
    ...
    auto no_lakes = [this]( const point_abs_om & pt ) {
        const auto settings_it = region_settings_map.find( "default" );
        if( settings_it == region_settings_map.end() ) {
            return true;
        }
        const regional_settings &settings = settings_it->second;
        ...
        if( val_emplaced.second ) {
            const overmap_lake_settings &lake_settings = settings.overmap_lake;
    ...
    if( intersection_candidates.empty() ) {
        // Extremely water-heavy worlds can leave the configured variance radius with no
        // legal intersection candidate.  Do not pass an empty vector to random_entry().
        // First try the grid origin, then widen the search a little while preserving the
        // deterministic grid topology.  The final fallback keeps the node usable even if
        // every nearby overmap is classified as lake.
        if( no_lakes( grid_pos ) ) {
            node.set_offset_pos( grid_pos );
            return;
        }
        for( int expanded_radius = max_offset_variance + 1;
             expanded_radius <= max_offset_variance + 4 && intersection_candidates.empty();
             ++expanded_radius ) {
            for( const tripoint_abs_om &p : points_in_radius( as_tripoint, expanded_radius ) ) {
                const point_abs_om candidate = p.xy();
                if( candidate != grid_pos && no_lakes( candidate ) ) {
                    intersection_candidates.emplace_back( candidate );
                }
            }
        }
        if( intersection_candidates.empty() ) {
            debugmsg( "no dry highway intersection could be generated near overmap %s; using the grid origin",
                      grid_pos.to_string() );
            node.set_offset_pos( grid_pos );
            return;
        }
